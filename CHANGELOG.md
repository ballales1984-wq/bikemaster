...
- Wahoo integration (ROADMAP #171): `bike_analyzer/backend/ingestion/wahoo_client.py` + route `/api/v1/import/wahoo/{auth,callback,sync,disconnect}` mirroring Strava/Garmin/Google Fit flow; frontend option in `ImportPanel.vue` (i18n it/en)
- AI Coach voice input/output (ROADMAP #189): Web Speech API `SpeechRecognition` (voice input) and `SpeechSynthesis` (voice output) in `CoachPanel.vue`; advanced prompt engineering in `ai_coach.py` (structured system prompt, few-shot examples, athlete-context handling) with local fallback preserved when `AI_COACH_MODE=local/offline`
- Config decoupling (ROADMAP #234): legacy `config.py` no longer imported anywhere; all settings flow through `settings.py`/`os.getenv` (file deletion pending OS file lock)

### Fixed

- Deploy: removed the separate Prometheus HTTP server on `:9090` so the container exposes a single port (`:10000`); metrics remain available at `/metrics` on the main app. Resolves Render health-check timeout caused by the dual-port binding

### Changed

- `ai_coach.py`: `chat_with_tools` selects the model per provider via explicit `if/elif/else` (fixes mypy `has-type` on `model`)
...
- Phone GPS follow-up (ROADMAP #224–#228) for the Android Capacitor app (`frontend/android/`)
- #224 Activity recognition: `BikeActivityRecognizer.kt` uses Google Play `ActivityRecognitionClient` (Transition API) with a GPS-speed heuristic fallback (`DetectedActivityType.fromSpeed`); detected segments are written into the ride GPX `<extensions>` block as `<bikemaster:activity>`
- #225 Automatic upload: `RideUploader.kt` uploads the finished ride to `POST /api/v1/import/gpx` (multipart, `Authorization: Bearer`) on stop, with exponential-backoff retries (3 attempts) and an offline queue (`pending_uploads.json`) flushed on next start
- #226 Completion push notification: a separate `bikemaster_uploads` channel posts a success/error local notification (`NotificationCompat`) after each upload
- #227 Kotlin unit tests: `GpsCompressionTest.kt` (Douglas-Peucker port mirroring backend `gps_parser.douglas_peucker`, haversine, path length) and `RideMappingTest.kt` (activity mapping + GPX extension/JSON builders)
- #228 Instrumented road test: `BikeTrackingInstrumentedTest.kt` (Espresso) + `frontend/android/ROAD_TEST.md` documenting the on-device procedure
- Capacitor plugin `BikeTrackingPlugin` now forwards `authToken` / `apiBaseUrl` / `rideName` to the service and returns `activities`, `uploadStatus` and `rideId` on stop
- New permissions in `AndroidManifest.xml`: `ACTIVITY_RECOGNITION`, `POST_NOTIFICATIONS`, `ACCESS_NETWORK_STATE`

### Changed

- `BikeTrackingService.kt` now records activity segments, auto-uploads on stop and emits the completion notification; activity-extension/JSON building extracted to pure `GpxExtensions.kt`
- `build.gradle` adds `play-services-location`, JUnit/Espresso test deps, `testInstrumentationRunner` and `repositories`

### Technical Details

- GPS compression matches the backend decimation (tolerance 0.00005) so mobile and server processing are consistent
- Build requires Android SDK + Capacitor sync (`npx cap sync android`); the committed `frontend/android/` lacks `settings.gradle`/`gradlew` (generated by Capacitor), so Gradle compilation was not run in this environment — prerequisites are documented in `frontend/android/ROAD_TEST.md`
- Upload target is the existing backend endpoint `POST /api/v1/import/gpx` (referenced as `/api/v1/rides/import` in the roadmap)
