...
class TrainingLoadModel(Algorithm):
    ...
    def _compute(self, ctx: AnalysisContext, extra: Optional[dict]) -> tuple[float, float, float]:
        ...
        if not history:
            ...
            tss = self._estimate_tss(m["duration_s"], avg_power, ftp)
            ctl = tss * 0.3
            atl = tss * 0.6
            tsb = ctl - atl
            precision = max(tss * 0.2, 5.0)
            confidence = 0.5
        ...
        for act in history:
            ...
            tss_list.append(self._estimate_tss(dur, avg_pwr, ftp))

        alpha_ctl = 2.0 / (42.0 + 1.0)
        alpha_atl = 2.0 / (7.0 + 1.0)
        ctl_series = self._ema(tss_list, alpha_ctl)
        atl_series = self._ema(tss_list, alpha_atl)
    ...
    def _extra_details(self, ctx: AnalysisContext, extra: Optional[dict]) -> dict:
        ...
        if not tss_history:
            ...
            pts = [p for p in ctx.activity.points if p.power is not None]
            avg_power = sum(p.power for p in pts) / len(pts) if pts else ftp * 0.7
            tss_history = [self._estimate_tss(m["duration_s"], avg_power, ftp)]
