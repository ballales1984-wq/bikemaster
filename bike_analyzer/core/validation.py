...
class ValidatedGPSPoint(BaseModel):
    ...
    speed: Optional[float] = Field(None, ge=0, le=120)
    power: Optional[float] = Field(None, ge=0, le=2000)
    heart_rate: Optional[int] = Field(None, ge=30, le=220)
    cadence: Optional[int] = Field(None, ge=0, le=200)
...
class ValidatedRide(BaseModel):
    ...
    avg_speed_kmh: Optional[float] = Field(None, ge=0, le=80)
    elevation_gain_m: Optional[float] = Field(None, ge=0, le=15000)
...
class ValidatedAthleteProfile(BaseModel):
    ...
    ftp_watts: Optional[int] = Field(None, ge=50, le=600)
    experience_level: str = Field(
        ..., pattern="^(Beginner|Intermediate|Advanced|Elite)$"
    )
