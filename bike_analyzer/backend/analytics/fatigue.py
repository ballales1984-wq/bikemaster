...
def calculate_fatigue_score(ride: Ride, rider_age: int = 35) -> float:
    ...
    if hr_avg:
        ...
        INTENSITY_FACTOR = 0.5 if hr_pct <= 0.5 else 0.5 + (hr_pct - 0.5) * 2.0 if hr_pct <= 0.85 else 1.5 + (hr_pct - 0.85) * 3.33
    else:
        INTENSITY_FACTOR = 1.0
    SPEED_FACTOR = min(ride.avg_speed_kmh / 25.0, 2.0)
    ELEV_FACTOR = 1.0 + min((ride.elevation_gain_m / ride.distance_km) / 20.0, 1.0) if ride.elevation_gain_m and ride.distance_km > 0 else 1.0
    WEIGHT_FACTOR = ride.weight_kg / 70.0
    return min((DURATION_FACTOR * 0.3 + INTENSITY_FACTOR * 0.3 + SPEED_FACTOR * 0.2 + ELEV_FACTOR * 0.1 + WEIGHT_FACTOR * 0.1) * 3.0, 10.0)
