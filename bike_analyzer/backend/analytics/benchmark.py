...
AGE_CATEGORIES = {"Under25": (0, 25), "25-35": (25, 35), "35-45": (35, 45), "45-55": (45, 55), "Over55": (55, 100)}
WEIGHT_CATEGORIES = {"Lightweight": (0, 65), "Medium": (65, 80), "Heavy": (80, 150)}
EXPERIENCE_CATEGORIES = {"Beginner": (0, 1), "Novice": (1, 3), "Experienced": (3, 7), "Veteran": (7, 20), "Legend": (20, 100)}

def compare_athlete_to_benchmark(athlete: AthleteProfile, total_km: float, avg_speed: float, total_hours: float) -> Dict[str, Optional[float]]:
    level = athlete.experience_level
    if level not in BENCHMARK_DATA: return {}
    bench = BENCHMARK_DATA[level]
    pct_km = min(100, (total_km - bench["total_km"][0]) / (bench["total_km"][1] - bench["total_km"][0]) * 100) if bench["total_km"][1] > bench["total_km"][0] else 50
    pct_speed = min(100, (avg_speed - bench["avg_speed"][0]) / (bench["avg_speed"][1] - bench["avg_speed"][0]) * 100) if bench["avg_speed"][1] > bench["avg_speed"][0] else 50
...
def generate_benchmark_report(athlete: AthleteProfile, rides: List) -> str:
    ...
    return f"Benchmark Report\nAtleta: {athlete.name}\nLivello: {athlete.experience_level}\nEtà: {athlete.age} ({get_age_category(athlete.age)})\nPeso: {athlete.weight_kg}kg ({get_weight_category(athlete.weight_kg)})\nEsperienza: {athlete.years_active} anni ({get_experience_category(athlete.years_active)})\nPercentile: {comparison.get('overall_percentile', 0)}%\nTotale km: {total_km:.0f} (media livello: {BENCHMARK_DATA.get(athlete.experience_level, {}).get('total_km', (0,0))})"