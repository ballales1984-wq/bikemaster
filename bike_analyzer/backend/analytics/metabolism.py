...
def resolve_reference_value(athlete: dict, profile: dict | None, tenant_id: int = 0) -> dict:
    ...
    imported = MetabolismRepository.get_metabolic_reference_value(sex, age, weight, activity_level, tenant_id)
...
def get_athlete_weights(athlete_id: int, tenant_id: int = 0) -> AdaptiveWeights:
    ...
    row = MetabolismRepository.get_metabolic_adaptive_weights(athlete_id, tenant_id)
...
def calibrate_athlete(
    ...
    MetabolismRepository.save_metabolic_adaptive_weights(calibrated.to_dict(), athlete_id, tenant_id)
    return {
...
def ensure_metabolic_profile(athlete_id: int, tenant_id: int = 0) -> dict:
    ...
    existing = MetabolismRepository.get_metabolic_profile(athlete_id, tenant_id)
    if existing:
        return existing
    return {}
...
def recalculate_daily_summary(athlete_id: int, date: str, tenant_id: int = 0) -> dict:
    ...
    if not profile and athlete:
        ...
        MetabolismRepository.save_metabolic_profile(profile, athlete_id, tenant_id)
        profile = MetabolismRepository.get_metabolic_profile(athlete_id, tenant_id) or profile
    rides = MetabolismRepository.get_rides_by_athlete(athlete_id, tenant_id)
    day_rides = [r for r in rides if r.get("date") == date]
    activity_level = profile.get("activity_level", "moderate") if profile else "moderate"
