...
def calculate_pace_consistency(segments: List[Segment]) -> Dict[str, float]:
    ...
    if not speeds or len(speeds) < 2:
        return {"cv_percent": 0.0, "min_speed": 0.0, "max_speed": 0.0, "pace_strategy": "unknown", "negative_split": False}
    mean_spd = sum(speeds) / len(speeds)
    variance = sum((s - mean_spd) ** 2 for s in speeds) / len(speeds)
    std_dev = math.sqrt(variance)
    if mean_spd > 0:
        cv = (std_dev / mean_spd) * 100
    else:
        cv = 0.0
    if cv < 10:
        strategy = "steady"
...
def calculate_power_estimate(
    ...
    if v_ms <= 0 or ride.duration_minutes <= 0:
        return {"power_avg_w": 0.0, "power_per_kg_w": 0.0, "w_prime_j": 0.0, "cd_wind": 0.0, "cd_rolling": 0.0, "cd_gravity": 0.0}
...
def estimate_vo2max(avg_speed_kmh: float, avg_gradient_percent: float, weight_kg: float, age: int = 35) -> Dict[str, float]:
    ...
    if vo2 < 40:
        ...
        level = "Excellent"
    return {"vo2_max_ml_kg_min": round(vo2, 1), "fitness_level": level, "age": age, "speed_match_kmh": round(speed_match_kmh, 1)}


def classify_ride_difficulty(ride: Ride) -> Dict[str, Any]:
    if ride.distance_km <= 0:
        return {"score": 0, "level": "unknown", "factors": {}}
    grade_factor = min((ride.elevation_gain_m / ride.distance_km) / 30.0, 1.0) if ride.elevation_gain_m and ride.distance_km > 0 else 0
...
def analyze_speed_profile(points: List[GPSPoint]) -> Dict[str, Any]:
    ...
    if not speeds or len(speeds) < 2:
        return {"acceleration_events": 0, "deceleration_events": 0, "speed_variance": 0.0, "coasting_time_pct": 0.0}
    thr_acc = 2.0
    thr_dec = -2.0
...
def calculate_progress_trend(rides: List[Ride], metric: str = "avg_speed_kmh") -> Dict[str, Any]:
    if not rides or len(rides) < 2:
        return {"trend": "insufficient_data", "slope": 0.0, "r_squared": 0.0, "improvement_pct": 0.0}
    ...
    if slope > 0.05:
        ...
        trend = "stable"
    return {"trend": trend, "slope": round(slope, 4), "r_squared": round(r_squared, 3), "improvement_pct": round(improvement, 1), "first_value": round(values[0], 2), "last_value": round(values[-1], 2), "data_points": n}


def calculate_training_stress_balance(rides: List[Ride], atl_tau_days: float = 7.0, ctl_tau_days: float = 42.0) -> Dict[str, Any]:
    if not rides:
    ...
    for i, date in enumerate(all_dates):
        ...
        if tsb > 10:
            ...
            form = "burnout_risk"
        result.append({"date": date, "tss": round(tss, 1), "atl": round(atl, 1), "ctl": round(ctl, 1), "tsb": round(tsb, 1), "form": form})
    latest = result[-1]
    return {"atl": latest["atl"], "ctl": latest["ctl"], "tsb": latest["tsb"], "form": latest["form"], "daily_load": result[-14:]}
...
def calculate_garmin_power_factor(ride: Ride, weight_kg: Optional[float] = None) -> Dict[str, float]:
    ...
    return {"pf": round(pf, 2), "np_w": round(np, 0), "if": round(if_val, 2), "tss_est": round(tss, 0)}


def calculate_heart_rate_zones(max_hr: int = 180, lthr: int = 155, current_avg_hr: Optional[float] = None) -> Dict[str, Dict[str, Any]]:
    zones: Dict[str, Dict[str, Any]] = {
        "Z1 (Recovery)": {"min": int(max_hr * 0.55), "max": int(max_hr * 0.64), "benefit": "Active recovery, warm-up"},
        "Z2 (Endurance)": {"min": int(max_hr * 0.64), "max": int(max_hr * 0.74), "benefit": "Aerobic base, fat burning"},
        "Z3 (Tempo)": {"min": int(max_hr * 0.74), "max": int(max_hr * 0.84), "benefit": "Sustained threshold effort"},
        "Z4 (Threshold)": {"min": int(max_hr * 0.84), "max": int(max_hr * 0.94), "benefit": "Lactate threshold, FTP improvement"},
        "Z5 (VO2max)": {"min": int(max_hr * 0.94), "max": max_hr, "benefit": "VO2max intervals, anaerobic"},
    }
...
def calculate_ride_recommendation_score(ride: Ride, athlete_weekly_km: float = 50.0) -> Dict[str, Any]:
    ...
    elevation_score = min((ride.elevation_gain_m / ride.distance_km) / 20.0 * 10, 10) if ride.elevation_gain_m and ride.distance_km and ride.distance_km > 0 else 0
    ...
    return {"overall_score": round(total, 1), "label": label, "volume_score": round(volume_score, 1), "intensity_score": round(intensity_score, 1), "elevation_score": round(elevation_score, 1)}


def detect_speed_surges(points: List[GPSPoint], threshold_kmh: float = 5.0, min_speed_kmh: float = 15.0) -> List[Dict[str, Any]]:
    if len(points) < 2:
        return []
    surges = []
    for i in range(1, len(points)):
        ...
        if prev.speed is not None and curr.speed is not None:
            ...
            if delta >= threshold_kmh and curr.speed >= min_speed_kmh:
                surges.append({"index": i, "speed_jump_kmh": round(delta, 1), "speed_kmh": round(curr.speed, 1)})
    return surges
