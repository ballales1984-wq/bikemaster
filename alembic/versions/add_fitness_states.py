...
def upgrade() -> None:
    ...
    if not inspector.has_table("fitness_states"):
        op.create_table(
            "fitness_states",
    ...
    _idx = {idx["name"] for idx in inspector.get_indexes("fitness_states")} if inspector.has_table("fitness_states") else set()
    if "ix_fitness_states_athlete_date" not in _idx:
        op.create_index("ix_fitness_states_athlete_date", "fitness_states", ["athlete_id", "date"])
    if "ix_fitness_states_ctl" not in _idx:
