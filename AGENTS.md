...
> **⚠️ Nota persistenza** — su Render il container è configurato con un disk persistente da 1 GB montato su `/mnt/data` (vedi `render.yaml`). Il percorso `DB_PATH=/mnt/data/rides.db` è quindi persistente tra i resume. I domini migrati su PostgreSQL (`athlete`, `rides`, `metrics`, `training_stress`, `itineraries`, `training_goals`) sono comunque protetti perché il layer sincrono (`db/database.py`) effettua il dispatch automatico verso i moduli Postgres quando `DATABASE_URL` è configurato (vedi `has_postgres()`). I domini rimanenti SQLite-only (es. POI, metabolico, chat, calendario, weather, BLE, sensor) usano lo stesso disk `/mnt/data` e sono quindi anch'essi protetti dalla perdita dati su Render, a patto che il disk sia effettivamente montato e il piano Render lo supporti.
>
...
>
> **Rimangono SQLite-only** (nessun dispatch guard, ma dati sul disk persistente `/mnt/data` su Render):
