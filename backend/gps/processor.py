...
Handles cleaning, interpolation, segment calculation, elevation, speed.
"""
...
def remove_duplicates(lats: np.ndarray, lons: np.ndarray, tol_m: float = 2.0) -> np.ndarray:
    if len(lats) < 2:
        return np.arange(len(lats))
    d = haversine_m(lats[:-1], lons[:-1], lats[1:], lons[1:])
    keep = np.empty(len(lats), dtype=bool)
    keep[0] = True
    keep[1:] = d > tol_m
    keep[-1] = True
...
def remove_stationary(lats: np.ndarray, lons: np.ndarray, timestamps: np.ndarray,
                      min_speed_ms: float = 0.5) -> np.ndarray:
    if len(lats) < 3:
        return np.arange(len(lats))
    d = haversine_m(lats[:-1], lon1=lons[:-1], lat2=lats[1:], lon2=lons[1:])
...
def compute_speeds(dist_m: np.ndarray, dt_s: np.ndarray) -> np.ndarray:
    dt = np.maximum(dt_s, 0.1)
    return (dist_m / dt) * 3.6


def compute_elevation_changes(elev: np.ndarray) -> tuple[float, float]:
    diffs = np.diff(elev)
...
def interpolate_gps(lats: np.ndarray, lons: np.ndarray, timestamps: np.ndarray,
                    interval_s: float = 5.0) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    if len(lats) < 2:
        return lats, lons, timestamps
    t = timestamps.astype("datetime64[s]").astype(float)
    cs_lat = CubicSpline(t, lats)
    cs_lon = CubicSpline(t, lons)
    t_new = np.arange(t[0], t[-1], interval_s)
    if len(t_new) < 2:
        return lats, lons, timestamps
    lats_i = cs_lat(t_new)
    lons_i = cs_lon(t_new)
    timestamps_i = np.array([np.datetime64(int(x), "s").astype("datetime64[us]").astype(datetime) for x in t_new])
    return lats_i, lons_i, timestamps_i


class GPSProcessor:
    def __init__(self, max_jump_m: float = 500.0, dup_tol_m: float = 2.0,
                 min_speed_ms: float = 0.5, interpolate_interval: Optional[float] = 5.0,
                 smooth_window: int = 3):
        self.max_jump_m = max_jump_m
        self.dup_tol_m = dup_tol_m
    ...
    def process(self, lats: list[float], lons: list[float],
        ...
        elev_arr = np.array(elevations, dtype=np.float64) if elevations and any(e is not None for e in elevations) else None
        ...
        lat_c, lon_c, ts_c = lat_arr[idx], lon_arr[idx], ts_arr[idx]

        if self.interpolate_interval and len(lat_c) >= 2:
            lat_c, lon_c, ts_c = interpolate_gps(lat_c, lon_c, ts_c, self.interpolate_interval)
            if elev_arr is not None:
                t_old = ts_arr.astype("datetime64[s]").astype(float)[idx]
                cs_elev = CubicSpline(t_old, elev_arr[idx])
                t_new = ts_c.astype("datetime64[s]").astype(float)
                elev_c = cs_elev(t_new)
            else:
                elev_c = None
        else:
        ...
        if self.smooth_window > 1 and len(speeds) >= self.smooth_window:
        ...
        if elev_c is not None:
            gain, loss = compute_elevation_changes(elev_c)
            elev_data = {
            ...
            elev_data = {"min_elevation": None, "max_elevation": None, "elevation_gain": 0.0, "elevation_loss": 0.0}
    ...
    def _clean_indices(self, lats, lons, timestamps) -> np.ndarray:
        ...
        idx = idx[remove_duplicates(lats, lons, self.dup_tol_m)]
        if len(idx) < 2:
            return idx
        idx = idx[remove_jumps(lats[idx], lons[idx], self.max_jump_m)]
        if len(idx) < 2:
            return idx
        idx = idx[remove_stationary(lats[idx], lons[idx], timestamps[idx], self.min_speed_ms)]
        return idx
