...
class TestAdvancedEdgeCases:
    ...
    def test_ride_difficulty_extreme(self):
        ride = Ride(date="2024-06-01", distance_km=200, duration_minutes=480, avg_speed_kmh=35, heart_rate_avg=185, elevation_gain_m=5000)
    ...
    def test_elevation_profile_all_flat(self):
        points = [
            ...
            GPSPoint(46.1, 11.2, datetime.now(timezone.utc), altitude=100),
            GPSPoint(46.1, 11.3, datetime.now(timezone.utc), altitude=100),
        ]
    ...
    def test_elevation_profile_insufficient_points(self):
        ...
        result = analyze_elevation_profile([GPSPoint(46.1, 11.1, datetime.now(timezone.utc))])
        assert result["hardship_index"] == 0.0

    def test_speed_profile_coasting(self):
        points = [
            GPSPoint(46.1, 11.1, datetime.now(timezone.utc), speed=25.0),
            GPSPoint(46.1, 11.2, datetime.now(timezone.utc), speed=25.0),
            GPSPoint(46.1, 11.3, datetime.now(timezone.utc), speed=25.0),
        ]
    ...
    def test_speed_profile_insufficient(self):
        ...
        pts = [GPSPoint(46.1, 11.1, datetime.now(timezone.utc), speed=25.0)]
    ...
    def test_progress_trend_error_handling(self):
        ...
        rides = [Ride(date="2024-01-01", avg_speed_kmh=None), Ride(date="2024-02-01", avg_speed_kmh=None)]
        result = calculate_progress_trend(rides)
        assert result["trend"] == "insufficient_data"
    ...
    def test_ride_recommendation_score_race(self):
        ride = Ride(date="2024-06-01", distance_km=100, duration_minutes=240, avg_speed_kmh=35, elevation_gain_m=2000)
        result = calculate_ride_recommendation_score(ride)
    ...
    def test_detect_speed_surges(self):
        points = [
            ...
            GPSPoint(46.1, 11.2, datetime.now(timezone.utc), speed=25),
            GPSPoint(46.1, 11.3, datetime.now(timezone.utc), speed=32),
            GPSPoint(46.1, 11.4, datetime.now(timezone.utc), speed=15),
        ]
    ...
    def test_detect_speed_surges_insufficient(self):
        ...
        assert detect_speed_surges([GPSPoint(46.1, 11.1, datetime.now(timezone.utc))]) == []
...
class TestPerformanceEdgeCases:
    ...
    def test_efficiency_score(self):
        ride = Ride(date="2024-06-01", distance_km=30, duration_minutes=90, avg_speed_kmh=25, calories=400)
        score = calculate_efficiency_score(ride)
        assert score.score > 0

    def test_monthly_summary(self):
        rides = [
            Ride(date="2024-06-05", distance_km=30, duration_minutes=90, avg_speed_kmh=25, calories=400),
            Ride(date="2024-06-15", distance_km=40, duration_minutes=100, avg_speed_kmh=28, calories=500),
        ]
    ...
    def test_classify_athlete_elite(self):
        rides = [Ride(date=f"2024-{i:02d}-01", distance_km=100, duration_minutes=200, avg_speed_kmh=25) for i in range(1, 35)]
        result = classify_athlete(rides)
        assert result in ("Elite", "Advanced", "Intermediate")

    def test_classify_athlete_beginner(self):
        rides = [Ride(date=f"2024-06-{i:02d}", distance_km=10, duration_minutes=30, avg_speed_kmh=15) for i in range(1, 5)]
        result = classify_athlete(rides)
...
class TestTrainingLoadExtendedBranches:
    def test_rss_with_elevation_factor(self):
        ride = Ride(date="2024-06-01", distance_km=10, duration_minutes=60, avg_speed_kmh=15, elevation_gain_m=400)
        tss = calculate_rss(ride)
        assert tss > 0

    def test_rss_with_hr_intensity(self):
        ride = Ride(date="2024-06-01", distance_km=20, duration_minutes=60, avg_speed_kmh=20, heart_rate_avg=175)
        tss = calculate_rss(ride)
        assert tss > 0
    ...
    def test_training_status_overreached(self):
        rides = [
            Ride(date="2024-06-15", distance_km=50, duration_minutes=180, avg_speed_kmh=28, heart_rate_avg=175),
            Ride(date="2024-06-14", distance_km=50, duration_minutes=180, avg_speed_kmh=28, heart_rate_avg=175),
            Ride(date="2024-06-13", distance_km=50, duration_minutes=180, avg_speed_kmh=28, heart_rate_avg=175),
            Ride(date="2024-06-12", distance_km=50, duration_minutes=180, avg_speed_kmh=28, heart_rate_avg=175),
            Ride(date="2024-06-11", distance_km=50, duration_minutes=180, avg_speed_kmh=28, heart_rate_avg=175),
            Ride(date="2024-06-10", distance_km=50, duration_minutes=180, avg_speed_kmh=28, heart_rate_avg=175),
            Ride(date="2024-06-09", distance_km=50, duration_minutes=180, avg_speed_kmh=28, heart_rate_avg=175),
        ]
...
class TestAnalyticsLogicBranches:
    def test_analyze_ride_complete(self):
        ride = Ride(date="2024-06-01", distance_km=30, duration_minutes=90,
                    avg_speed_kmh=25, calories=450, heart_rate_avg=155, elevation_gain_m=200)
    ...
    def test_calculate_summary_single_ride(self):
        rides = [Ride(date="2024-06-01", distance_km=30, duration_minutes=90, avg_speed_kmh=25, calories=400)]
    ...
    def test_rides_to_csv_with_nulls(self):
        rides = [Ride(date="2024-06-01", distance_km=25, duration_minutes=60, heart_rate_avg=None, elevation_gain_m=None)]
