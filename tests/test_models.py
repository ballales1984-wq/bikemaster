...
def test_gps_point_distance():
    p1 = GPSPoint(lat=45.0, lon=9.0, timestamp=datetime(2024, 1, 1, tzinfo=UTC))
    p2 = GPSPoint(lat=45.01, lon=9.01, timestamp=datetime(2024, 1, 1, 0, 1, tzinfo=UTC))
    d = p1.distance_to(p2)
    assert 0 < d < 2000
...
def test_validate_coordinate_valid():
    assert validate_coordinate(45.0, 9.0)
    assert validate_coordinate(-90, 180)
    assert validate_coordinate(90, -180)


def test_validate_coordinate_invalid():
    assert not validate_coordinate(91, 9.0)
    assert not validate_coordinate(-91, 9.0)
    assert not validate_coordinate(45.0, 181)
    assert not validate_coordinate(45.0, -181)
    assert not validate_coordinate("a", 9.0)


def test_validate_gps_point():
    p = GPSPoint(lat=45.0, lon=9.0, timestamp=datetime(2024, 1, 1, tzinfo=UTC))
    assert validate_gps_point(p)
    p_invalid = GPSPoint(lat=91, lon=9.0, timestamp=datetime(2024, 1, 1, tzinfo=UTC))
    assert not validate_gps_point(p_invalid)
