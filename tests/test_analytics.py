...
def test_calculate_pace_consistency():
    from bike_analyzer.backend.analytics.advanced import calculate_pace_consistency
    from bike_analyzer.backend.models.models import Segment

    seg = Segment(
        start=GPSPoint(lat=45.0, lon=9.0, timestamp=datetime(2024, 1, 1, tzinfo=UTC)),
        end=GPSPoint(lat=45.1, lon=9.1, timestamp=datetime(2024, 1, 1, 0, 30, tzinfo=UTC)),
        distance_m=10000,
        duration_s=1800,
        avg_speed_km_h=20.0,
    )
    result = calculate_pace_consistency([seg, seg, seg])
    assert "cv_percent" in result
    assert result["pace_strategy"] in ["steady", "variable", "erratic"]


def test_calculate_pace_consistency_empty():
    from bike_analyzer.backend.analytics.advanced import calculate_pace_consistency
...
def test_calculate_power_estimate():
    from bike_analyzer.backend.analytics.advanced import calculate_power_estimate
    ...
    result = calculate_power_estimate(r)
    assert "power_avg_w" in result
    assert result["power_avg_w"] > 0


def test_calculate_power_estimate_zero_speed():
    from bike_analyzer.backend.analytics.advanced import calculate_power_estimate

    r = Ride(date="2024-06-01", distance_km=25.0, avg_speed_kmh=0.0)
    result = calculate_power_estimate(r)
    assert result["power_avg_w"] == 0.0


def test_classify_climb():
    from bike_analyzer.backend.analytics.advanced import classify_climb

    result = classify_climb(5.0, 8.0)
    assert result["category"] == "3"

    result = classify_climb(0.2, 1.0)
    assert result["category"] == "none"


def test_classify_climb_short():
    from bike_analyzer.backend.analytics.advanced import classify_climb

    result = classify_climb(0.2, 10.0)
    assert result["category"] == "none"


def test_estimate_vo2max():
    from bike_analyzer.backend.analytics.advanced import estimate_vo2max

    result = estimate_vo2max(25.0, 5.0, 70.0, age=30)
    assert "vo2_max_ml_kg_min" in result
    assert result["fitness_level"] in ["Below Average", "Average", "Good", "Very Good", "Excellent"]


def test_estimate_vo2max_older():
    from bike_analyzer.backend.analytics.advanced import estimate_vo2max

    result = estimate_vo2max(20.0, 3.0, 70.0, age=50)
    assert result["age"] == 50


def test_classify_ride_difficulty():
    from bike_analyzer.backend.analytics.advanced import classify_ride_difficulty
    ...
    result = classify_ride_difficulty(r)
    assert "score" in result
    assert "level" in result


def test_classify_ride_difficulty_zero_distance():
    from bike_analyzer.backend.analytics.advanced import classify_ride_difficulty

    r = Ride(date="2024-06-01", distance_km=0.0)
    result = classify_ride_difficulty(r)
    assert result["level"] == "unknown"


def test_analyze_elevation_profile():
    from bike_analyzer.backend.analytics.advanced import analyze_elevation_profile

    pts = [
        GPSPoint(lat=45.0, lon=9.0, timestamp=datetime(2024, 1, 1, tzinfo=UTC), altitude=100.0),
        GPSPoint(lat=45.01, lon=9.01, timestamp=datetime(2024, 1, 1, 0, 1, tzinfo=UTC), altitude=150.0),
        GPSPoint(lat=45.02, lon=9.02, timestamp=datetime(2024, 1, 1, 0, 2, tzinfo=UTC), altitude=120.0),
    ]
...
def test_analyze_speed_profile():
    from bike_analyzer.backend.analytics.advanced import analyze_speed_profile

    pts = [
        GPSPoint(lat=45.0, lon=9.0, timestamp=datetime(2024, 1, 1, tzinfo=UTC), speed=20.0),
        GPSPoint(lat=45.01, lon=9.01, timestamp=datetime(2024, 1, 1, 0, 1, tzinfo=UTC), speed=22.0),
        GPSPoint(lat=45.02, lon=9.02, timestamp=datetime(2024, 1, 1, 0, 2, tzinfo=UTC), speed=25.0),
    ]
    result = analyze_speed_profile(pts)
    assert "acceleration_events" in result


def test_analyze_speed_profile_empty():
    from bike_analyzer.backend.analytics.advanced import analyze_speed_profile
...
def test_calculate_progress_trend():
    ...
    rides = [
        Ride(date="2024-06-01", distance_km=20.0, duration_minutes=60.0, avg_speed_kmh=20.0),
        Ride(date="2024-06-02", distance_km=25.0, duration_minutes=60.0, avg_speed_kmh=25.0),
        Ride(date="2024-06-03", distance_km=30.0, duration_minutes=60.0, avg_speed_kmh=30.0),
    ]
    result = calculate_progress_trend(rides)
    assert result["trend"] in ["improving", "declining", "stable", "insufficient_data"]


def test_calculate_progress_trend_single():
    from bike_analyzer.backend.analytics.advanced import calculate_progress_trend

    rides = [Ride(date="2024-06-01", distance_km=20.0, avg_speed_kmh=20.0)]
    result = calculate_progress_trend(rides)
    assert result["trend"] == "insufficient_data"


def test_estimate_ideal_weight():
    from bike_analyzer.backend.analytics.advanced import estimate_ideal_weight

    result = estimate_ideal_weight(ftp=280, height_cm=180, experience="Intermediate")
    assert result > 0


def test_estimate_ideal_weight_zero():
    from bike_analyzer.backend.analytics.advanced import estimate_ideal_weight

    result = estimate_ideal_weight(ftp=0, height_cm=180)
    assert result == 70.0


def test_calculate_training_stress_balance():
    from bike_analyzer.backend.analytics.advanced import calculate_training_stress_balance

    rides = [
        Ride(date="2024-06-01", distance_km=30.0, duration_minutes=90.0, avg_speed_kmh=20.0, calories=600),
    ]
...
def test_calculate_training_stress_balance_empty():
    from bike_analyzer.backend.analytics.advanced import calculate_training_stress_balance
...
def test_calculate_heart_rate_zones():
    ...
    result = calculate_heart_rate_zones(max_hr=180, lthr=155, current_avg_hr=150)
    assert "Z1 (Recovery)" in result
    assert "Z5 (VO2max)" in result
...
def test_calculate_ride_recommendation_score():
    ...
    r = Ride(date="2024-06-01", distance_km=30.0, avg_speed_kmh=25.0, elevation_gain_m=300.0)
    result = calculate_ride_recommendation_score(r)
...
def test_detect_speed_surges():
    ...
    pts = [
        GPSPoint(lat=45.0, lon=9.0, timestamp=datetime(2024, 1, 1, tzinfo=UTC), speed=15.0),
        GPSPoint(lat=45.01, lon=9.01, timestamp=datetime(2024, 1, 1, 0, 1, tzinfo=UTC), speed=25.0),
    ]
    result = detect_speed_surges(pts)
    assert len(result) == 1
    assert result[0]["speed_jump_kmh"] == 10.0


def test_detect_speed_surges_empty():
    from bike_analyzer.backend.analytics.advanced import detect_speed_surges
...
def test_compute_ctl_atl_tsb_external():
    ...
    rides = [
        Ride(date="2024-06-01", distance_km=30.0, calories=600),
    ]
