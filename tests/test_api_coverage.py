...
def test_benchmark_compare(client):
    r = client.post("/api/v1/benchmark/compare", json={
        "distance_km": 50.0,
...
def test_rides_report(client):
    ride = client.post("/api/v1/rides", json={
        "date": "2024-06-15",
        "distance_km": 25.0,
        "duration_minutes": 60,
        "calories": 400,
...
def test_athlete_scores(client):
    athlete = client.post("/api/v1/athletes", json={
        "name": "Score Test",
        "age": 25,
        "weight_kg": 65.0,
...
def test_ride_single_analyze(client):
    ...
    ride_id = ride.json()["id"]
    r = client.post(f"/api/v1/rides/{ride_id}/analyze", json={
        "date": "2024-06-15",
        "distance_km": 25.0,
        "duration_minutes": 60,
        "calories": 400,
...
def test_athlete_metrics(client):
    ...
    athlete_id = athlete.json()["id"]
    r = client.post(f"/api/v1/athletes/{athlete_id}/metrics", json={
        "type": "ftp",
        "value": 250.0,
    })
