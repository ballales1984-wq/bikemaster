(function () {
  ...
  function toggleTheme() {
    ...
    showToast(
      "Tema " + (next === "dark" ? "scuro" : "chiaro") + " attivo",
      "info",
    );
  }
  ...
  function createDrawer() {
    ...
    drawer.querySelectorAll(".tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        document
          .querySelector(
            '.tab[data-tab="' + tab.getAttribute("data-tab") + '"]',
          )
          ?.click();
        closeDrawer();
      });
  ...
  async function loadRides() {
    ...
    try {
      ...
      if (listEl)
        listEl.innerHTML =
          '<p class="empty-text">Impossibile caricare le ride. Riprova più tardi.</p>';
    } finally {
  ...
  function renderSkeletons(count) {
    return Array(count)
      .fill(0)
      .map(() => '<div class="ride-item skeleton skeleton-ride"></div>')
      .join("");
  }

  function updateStats(rides) {
    const totalKm = rides.reduce(
      (s, r) => s + (parseFloat(r.distance_km) || 0),
      0,
    );
    const totalCal = rides.reduce(
      (s, r) => s + (parseFloat(r.calories) || 0),
      0,
    );
    const avgSp = rides.length
      ? rides.reduce((s, r) => s + (parseFloat(r.avg_speed_kmh) || 0), 0) /
        rides.length
      : 0;
    const totalDur = rides.reduce(
      (s, r) => s + (parseFloat(r.duration_minutes) || 0),
      0,
    );
  ...
  function renderRides(rides) {
    ...
    if (rides.length === 0) {
      listEl.innerHTML =
        '<p class="empty-text">Nessuna ride. Aggiungine una sopra!</p>';
      return;
    }
    listEl.innerHTML = rides
      .map((r) => {
        ...
        return (
          '<div class="ride-item" role="listitem" tabindex="0" data-ride-id="' +
          r.id +
          '" aria-label="Ride del ' +
          escapeHtml(r.date) +
          ", " +
          escapeHtml(r.distance_km) +
          ' km">' +
          ...
          escapeHtml(r.distance_km) +
          " km &bull; " +
          escapeHtml(r.duration_minutes) +
          " min &bull; " +
          escapeHtml(r.avg_speed_kmh) +
          " km/h &bull; Fatigue: " +
          fatigue +
          "/10</div>" +
          "</div>" +
          '<div class="ride-item-actions">' +
          '<button class="btn btn-sm btn-secondary" data-action="detail" data-ride-id="' +
          r.id +
          '">Dettagli</button>' +
          '<button class="btn btn-sm btn-secondary" data-action="delete" data-ride-id="' +
          r.id +
          '">Elimina</button>' +
          "</div>" +
          "</div>"
    ...
    listEl.querySelectorAll('[data-action="detail"]').forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        onRideDetail(parseInt(btn.getAttribute("data-ride-id"), 10));
      });
    });
    listEl.querySelectorAll('[data-action="delete"]').forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        deleteRide(parseInt(btn.getAttribute("data-ride-id"), 10));
      });
    });
  }
  ...
  async function onFormSubmit(e) {
    ...
    try {
      const ride = {
        ...
        avg_speed_kmh:
          parseFloat(form.elements["avg_speed_kmh"].value) || undefined,
        calories: parseFloat(form.elements["calories"].value) || undefined,
        heart_rate_avg:
          parseFloat(form.elements["heart_rate_avg"].value) || null,
        elevation_gain_m:
          parseFloat(form.elements["elevation_gain_m"].value) || null,
      };
  ...
  function validateRideForm(form) {
    ...
    if (!dist || dist.value === "" || isNaN(parseFloat(dist.value)))
      ...
      errors["distance_km"] = "La distanza non può essere negativa";
    ...
    if (
      speed &&
      speed.value !== "" &&
      (isNaN(parseFloat(speed.value)) || parseFloat(speed.value) < 0)
    )
      errors["avg_speed_kmh"] = "Velocità non valida";

    if (
      hr &&
      hr.value !== "" &&
      (isNaN(parseFloat(hr.value)) ||
        parseFloat(hr.value) < 30 ||
        parseFloat(hr.value) > 220)
    )
    ...
    if (
      elev &&
      elev.value !== "" &&
      (isNaN(parseFloat(elev.value)) || parseFloat(elev.value) < 0)
    )
  ...
  async function onRideDetail(rideId) {
    ...
    try {
      ...
      const fatigue =
        ride.fatigue_score != null ? ride.fatigue_score.toFixed(1) : "0";
      const ck =
        ride.calories_per_km != null ? ride.calories_per_km.toFixed(0) : "0";
      detailContent.innerHTML =
        '<div class="detail-meta">' +
        "<div><strong>Data:</strong> " +
        escapeHtml(ride.date) +
        "</div>" +
        "<div><strong>Distanza:</strong> " +
        escapeHtml(ride.distance_km) +
        " km</div>" +
        "<div><strong>Durata:</strong> " +
        escapeHtml(ride.duration_minutes) +
        " min</div>" +
        "<div><strong>Vel Media:</strong> " +
        escapeHtml(ride.avg_speed_kmh) +
        " km/h</div>" +
        "<div><strong>Calorie:</strong> " +
        escapeHtml(ride.calories) +
        "</div>" +
        "<div><strong>HR Media:</strong> " +
        (ride.heart_rate_avg || "-") +
        " bpm</div>" +
        "<div><strong>Altitudine:</strong> " +
        (ride.elevation_gain_m || 0) +
        " m</div>" +
        "<div><strong>Fatigue:</strong> " +
        fatigue +
        "/10</div>" +
        "<div><strong>Cal/km:</strong> " +
        ck +
        "</div>" +
        "</div>";
      ...
      document
        .querySelectorAll(".chart-toolbar .btn-sm")
        .forEach((b) => b.setAttribute("data-ride", rideId));
  ...
  async function renderRideCharts(ride) {
    ...
    mkChart(
      "rideSpeedChart",
      "Velocità (km/h)",
      points.map((p) => (p.speed != null ? p.speed : 0)),
      "#FF6B00",
    );
    mkChart(
      "rideElevationChart",
      "Altitudine (m)",
      points.map((p) => (p.altitude != null ? p.altitude : 0)),
      "#4ecca3",
    );
    mkChart(
      "rideDistanceChart",
      "Distanza Cumulata (km)",
      points
        .map((p, i) => {
          if (i === 0) return 0;
          ...
          const aVal =
            ...
            Math.cos(toRad(prev.lat)) *
              ...
              Math.sin(dLon / 2) ** 2;
          return (
            (R * 2 * Math.atan2(Math.sqrt(aVal), Math.sqrt(1 - aVal))) / 1000
          );
        })
        .reduce((acc, v, i) => {
  ...
  function showRideMap(ride) {
    ...
    latLngs.forEach((ll, i) => {
      ...
      const alt =
        point.altitude != null ? point.altitude.toFixed(0) + " m" : "";
      const popupContent =
        "<strong>Punto " +
        (i + 1) +
        "</strong>" +
        (speed ? "<br>Velocità: " + escapeHtml(speed) : "") +
  ...
  function updateDurationChart(rides) {
    ...
    const data = rides
      .map((r) => parseFloat(r.duration_minutes) || 0)
      .slice(-10);

    if (durationChart) {
      const sameLabels =
        JSON.stringify(durationChart.data.labels) === JSON.stringify(labels);
      const sameData =
        JSON.stringify(durationChart.data.datasets[0].data) ===
        JSON.stringify(data);
  ...
  async function handleImportFiles(files) {
    ...
    progressEl.innerHTML =
      '<p class="loading-text">Importazione in corso...</p>';
    ...
    try {
      ...
      const count =
        result.count || result.imported ? result.imported.length : 0;
      progressEl.innerHTML =
        '<p style="color:var(--success)">Importati ' +
        count +
        " file con successo!</p>" +
        (result.imported || [])
          .map(
      ...
      progressEl.innerHTML =
        '<p style="color:var(--error)">Errore importazione: ' +
    ...
    if (saveBtn) {
      saveBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          document.getElementById("athlete-result").innerHTML =
            "<strong>Atleta creato:</strong> ID " + result.id;
      ...
      loadBtn.addEventListener("click", async () => {
        try {
          const rides = await apiGet(
            "/api/v1/rides?page=1&page_size=1&sort=date",
          );
          ...
          const athlete = await apiGet(
            "/api/v1/athletes/" + lastRide.athlete_id,
          );
  ...
  function setupCoachActions() {
    ...
    if (fullBtn) {
      fullBtn.addEventListener("click", async () => {
        const athleteId =
          parseInt(document.getElementById("coach-athlete-id").value, 10) || 0;
        try {
          ...
          const data = await apiGet(
            "/api/v1/coach/full?athlete_id=" + athleteId,
          );

          renderCoachScores(data.training_scores, "coach-scores");
          document.getElementById("coach-scores").classList.remove("hidden");

          document.getElementById("coach-training-advice").innerHTML =
            formatAdvice(data.training_advice);
          document.getElementById("coach-historical").innerHTML =
            data.historical_analysis
              ? '<div style="color:#4ecca3;font-size:1.1rem;font-weight:bold">' +
                escapeHtml(data.historical_analysis) +
                "</div>"
              : '<div style="color:#888">Dati insufficienti per trend</div>';
          document.getElementById("coach-recovery-advice").innerHTML =
            formatAdvice(data.recovery_advice);
          document
            .getElementById("coach-advice-panel")
        ...
        const cls =
          val >= 7
            ...
            : val >= 4
              ...
              : "score-value warning";
        return (
          '<div class="score-card"><div class="' +
          cls +
          '">' +
          formatNumber(val, 1) +
          '</div><div class="score-label">' +
          escapeHtml(s.label) +
          "</div></div>"
        );
      })
  ...
  function formatAdvice(text) {
    if (!text)
      return '<div style="color:#888">Nessun consiglio disponibile</div>';
    ...
    const items = lines
      .map((line) => {
        ...
        return (
          '<div class="advice-item"><span class="advice-num">' +
          escapeHtml(num) +
          "</span><span>" +
          escapeHtml(content) +
          "</span></div>"
    ...
    return (
      items.join("") ||
      '<div style="color:#888">Nessun consiglio disponibile</div>'
    );
  }
  ...
  function renderCoachCharts(charts) {
    ...
    if (!charts || !charts.length) {
      container.innerHTML =
        '<div style="color:#888;text-align:center;padding:20px">Nessun grafico disponibile</div>';
      container.classList.remove("hidden");
      return;
    }
    container.innerHTML = charts
      .map((url) => {
        ...
        return (
          '<div class="chart-card"><img src="' +
          url +
          "?t=" +
          ts +
          '" alt="Grafico AI Coach" style="max-width:100%;border-radius:8px" /></div>'
        );
      })
      .join("");
    container.classList.remove("hidden");
  ...
  function setupKnowledgeActions() {
    ...
    if (searchBtn) {
      searchBtn.addEventListener("click", async () => {
        ...
        if (!query) {
          showToast("Inserisci una query", "warning");
          return;
        }
        try {
  ...
  function setupBenchmark() {
    ...
    if (btn) {
      btn.addEventListener("click", async () => {
        const distance =
          parseFloat(document.getElementById("benchmark-distance").value) || 0;
        const speed =
          parseFloat(document.getElementById("benchmark-speed").value) || 0;
        const durationHours =
          parseFloat(document.getElementById("benchmark-duration").value) || 0;
        try {
  ...
  function setupRideFilters() {
    ...
    if (applyBtn) {
      applyBtn.addEventListener("click", () => {
        rideFilters.startDate =
          document.getElementById("filter-start-date")?.value || "";
        rideFilters.endDate =
          document.getElementById("filter-end-date")?.value || "";
        rideFilters.minDistance =
          document.getElementById("filter-min-distance")?.value || "";
        rideFilters.maxDistance =
  ...
  function filterRides(rides) {
    return rides.filter((r) => {
      ...
      if (rideFilters.minDistance && dist < parseFloat(rideFilters.minDistance))
        return false;
      if (rideFilters.maxDistance && dist > parseFloat(rideFilters.maxDistance))
        return false;
      return true;
    });
  }
