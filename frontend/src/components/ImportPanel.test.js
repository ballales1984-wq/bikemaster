...
vi.mock('../utils/api.ts', () => ({ apiGet, apiPost, apiPut, apiDelete, apiUpload }))
...
describe('ImportPanel', () => {
  ...
  it('displays selected file count after onChange', async () => {
    ...
    expect(wrapper.find('.upload-placeholder').text()).toContain('2 files selected')
  })
  ...
  it('imports GPX file via upload method', async () => {
    ...
    expect(apiUpload).toHaveBeenCalledWith('/api/v1/import/gpx', expect.any(File))
  ...
  it('imports FIT file via upload method', async () => {
    ...
    expect(apiUpload).toHaveBeenCalledWith('/api/v1/import/fit', expect.any(File))
  })
  ...
  it('imports multiple files with progress', async () => {
    ...
    wrapper.vm.files = [makeFile('a.gpx'), makeFile('b.fit'), makeFile('c.gpx')]
    await wrapper.vm.$nextTick()
