  ...
  <div v-if="auth.isAdmin" class="admin-bm2">
    ...
    <div class="bm2-tabs">
      <button
        v-for="tab in tabs"
        :key="tab.key"
        :class="['bm2-tab', { active: activeTab === tab.key }]"
    ...
    <section v-if="activeTab === 'catalog'" class="bm2-section">
      ...
      <div v-if="loadingCatalog" class="bm2-loading">
        {{ t("common.loading") }}
      </div>
      <div v-else class="bm2-catalog">
        <div v-for="model in catalog" :key="model.name" class="bm2-model-card">
          ...
          <div class="bm2-meta">
            <span
              >Inputs: {{ getModelInputs(model.name).join(", ") || "—" }}</span
            >
          </div>
        </div>
      </div>
      ...
      <form class="bm2-form" @submit.prevent="runTest">
        ...
        <div class="bm2-row">
          <label
            >Weight (kg)<input
              id="abm2-weight"
              v-model.number="testForm.weight"
              type="number"
          /></label>
          <label
            >Age<input
              id="abm2-age"
              v-model.number="testForm.age"
              type="number"
          /></label>
          <label
            >FTP (W)<input
              id="abm2-ftp"
              v-model.number="testForm.ftp"
              type="number"
          /></label>
          <label
            >Experience
        ...
        <div class="bm2-row">
          <label
            >Bike weight (kg)<input
              id="abm2-bike"
              v-model.number="testForm.bikeWeight"
              type="number"
          /></label>
          <label
            >Crr<input
              id="abm2-crr"
              v-model.number="testForm.crr"
              type="number"
              step="0.001"
          /></label>
          <label
        ...
        <div class="bm2-row">
          <label
            >Slope (%)<input
              id="abm2-slope"
              v-model.number="testForm.slope"
              type="number"
          /></label>
          <label
            >Wind (m/s)<input
              id="abm2-wind"
              v-model.number="testForm.wind"
              type="number"
          /></label>
          <label
            >Temp (°C)<input
              id="abm2-temp"
              v-model.number="testForm.temp"
              type="number"
          /></label>
        </div>
        <button type="submit" :disabled="testLoading">
          {{ testLoading ? t("common.loading") : t("admin.bm2.runTest") }}
        </button>
      </form>

      <div v-if="testResult" class="bm2-results">
        <div v-if="testResult.error" class="bm2-error">
          {{ testResult.error }}
        </div>
        <div v-else>
          <div class="bm2-models-used">
            <strong>{{ t("admin.bm2.modelsUsed") }}:</strong>
            {{
              ((testResult.models_used as string[]) || []).join(", ") || "none"
            }}
          </div>
          <article
            v-for="[name, r] in resultList(
              testResult.results as Record<string, Bm2ModelResult>,
            )"
            :key="name"
            class="bm2-card"
          >
            <header>
              ...
              <span class="bm2-value"
                >{{ (r as Bm2ModelResult).value.toFixed(1) }}
                {{ (r as Bm2ModelResult).unit }}</span
              >
            </header>
            <dl>
              <dt>Formula</dt>
              <dd>{{ (r as Bm2ModelResult).formula }}</dd>
              <dt>Data used</dt>
              <dd>{{ (r as Bm2ModelResult).data_used.join(", ") }}</dd>
              <dt>Precision</dt>
              <dd>
                ±{{ (r as Bm2ModelResult).precision.toFixed(2) }}
                {{ (r as Bm2ModelResult).unit }}
              </dd>
              <dt>Confidence</dt>
              <dd>{{ Math.round((r as Bm2ModelResult).confidence * 100) }}%</dd>
              <dt>Source</dt>
              <dd>{{ (r as Bm2ModelResult).source }}</dd>
              <template
                v-if="
                  (r as Bm2ModelResult).details &&
                  Object.keys((r as Bm2ModelResult).details!).length
                "
              >
                <dt>Details</dt>
                <dd
                  v-for="(val, key) in (r as Bm2ModelResult).details"
                  :key="key"
                >
                  {{ key }}: {{ formatDetail(val) }}
                </dd>
              </template>
            </dl>
          </article>
    ...
    <section v-if="activeTab === 'simulation'" class="bm2-section">
      ...
      <form class="bm2-form" @submit.prevent="runSimulation">
        <label
          >Ride ID<input
            id="abm2-ride-id"
            v-model.number="simForm.rideId"
            type="number"
            min="1"
        /></label>
        <div class="bm2-row">
          <label
            >Δ athlete weight (kg)<input
              id="abm2-awd"
              v-model.number="simForm.athleteWeightDelta"
              type="number"
              step="0.5"
          /></label>
          <label
            >Δ bike weight (kg)<input
              id="abm2-bwd"
              v-model.number="simForm.bikeWeightDelta"
              type="number"
              step="0.5"
          /></label>
          <label
            >Δ slope (%)<input
              id="abm2-sd"
              v-model.number="simForm.slopeDelta"
              type="number"
              step="0.5"
          /></label>
          <label
            >CdA override<input
              id="abm2-cda"
              v-model.number="simForm.cdaOverride"
              type="number"
              step="0.01"
          /></label>
        </div>
        <button type="submit" :disabled="simLoading">
          {{ simLoading ? t("common.loading") : t("admin.bm2.runSimulation") }}
        </button>
      </form>
      <div v-if="simResult" class="bm2-results">
        <div v-if="simResult.error" class="bm2-error">
          {{ simResult.error }}
        </div>
        <div v-else>
          ...
          <div class="bm2-comparison">
            <div
              v-for="[model, delta] in Object.entries(
                simResult.comparison.deltas,
              )"
              :key="model"
              class="bm2-delta-row"
            >
    ...
    <section v-if="activeTab === 'constants'" class="bm2-section">
      ...
      <div class="bm2-constants">
        ...
        <div class="bm2-const-block">
          ...
          <ul>
            <li v-for="(conf, src) in sourceConfidence" :key="src">
              <strong>{{ src }}</strong
              >: {{ conf }}
            </li>
          </ul>
        ...
        <div class="bm2-const-block">
          ...
          <ul>
            <li v-for="(range, unit) in rangeRules" :key="unit">
              <strong>{{ unit }}</strong
              >: [{{ range[0] }}, {{ range[1] }}]
            </li>
          </ul>
        ...
        <div class="bm2-const-block">
          ...
          <ul>
            ...
            <li>
              <strong>Ambiguous keywords</strong>:
...
// --- Catalog ---
const catalog = ref<
  Array<{ name: string; formula: string; unit: string; description: string }>
>([]);
...
function buildTestPayload() {
  ...
  return {
    ...
    athlete: {
      ...
      age: testForm.age,
      ftp: testForm.ftp,
      experience_level: testForm.experience,
      max_hr: 190,
    },
    bike: {
      weight: testForm.bikeWeight,
      crr: testForm.crr,
      cda: testForm.cda,
      drivetrain_efficiency: 0.97,
    },
    world: {
      surface: testForm.surface,
      avg_slope: testForm.slope,
      wind_speed: testForm.wind,
      temperature: testForm.temp,
    },
    gps_points: pts,
    sensors: [
...
function resultList(
  results: Record<string, Bm2ModelResult> | undefined,
): [string, Bm2ModelResult][] {
  if (!results) return [];
...
const simResult = ref<{
  ride_id: number | null;
  comparison: { deltas: Record<string, number> };
  summary: string;
  error?: string;
} | null>(null);
...
.bm2-form button[type="submit"]:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
.bm2-results {
