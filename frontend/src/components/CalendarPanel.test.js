...
const mockEvents = {
  events: [
    { id: 1, title: 'Morning Ride', event_type: 'training', date: '2026-06-20', completed: false },
    { id: 2, title: 'FTP Test', event_type: 'test', date: '2026-06-20', completed: false },
    { id: 3, title: 'Race Day', event_type: 'race', date: '2026-06-20', completed: true },
    { id: 4, title: 'Recovery', event_type: 'recovery', date: '2026-06-20', completed: false },
  ],
}
...
describe('CalendarPanel', () => {
  ...
  it('loads events and goals after athlete loads', async () => {
    ...
    const eventsCalls = apiGet.mock.calls.filter(c => c[0] === '/api/v1/calendar/events')
    expect(eventsCalls.length).toBeGreaterThan(0)
    const goalsCalls = apiGet.mock.calls.filter(c => c[0] === '/api/v1/athletes/1')
  ...
  it('saves new event via API', async () => {
    ...
    expect(apiPost).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ title: 'New Ride', event_type: 'training', athlete_id: 1 })
    )
  })
  ...
  it('weatherScoreClass returns valid score', async () => {
    ...
    const score = wrapper.vm.weatherScoreClass({ weather_temp: 20, weather_humidity: 50 })
  ...
  it('changes athlete selection', async () => {
    ...
    const calls = apiGet.mock.calls.filter(c => c[0] === '/api/v1/calendar/events' && c[1]?.athlete_id === 2)
