...
const rides = ref<
  Array<{ id: number; date: string; title: string; distance_km: number }>
>([]);
...
function buildOverride() {
  ...
  if (rideForm.cdaOverride != null)
    override.cda_override = rideForm.cdaOverride;
  return override;
}
...
function onSubmit() {
  ...
  if (
    isSimulation.value ||
    /se\s|what if|simula|ipotizz/i.test(form.question)
  ) {
    simulate(payload);
  } else {
    ask(payload);
  ...
  <section class="bm2-panel">
    ...
    <form class="bm2-form" @submit.prevent="onSubmit">
      <label
        >Domanda<input
          ...
          type="text"
          placeholder="Es. Quanta energia consumo?"
      /></label>
      <div class="bm2-row">
        <label
          >Peso atleta (kg)<input
            id="bm2-weight"
        ...
        <label
          >Peso bici (kg)<input
            id="bm2-bike-weight"
            v-model.number="form.bikeWeight"
            type="number"
        /></label>
        <label
          >Pendenza (%)<input
            id="bm2-slope"
            v-model.number="form.slope"
            type="number"
        /></label>
        <label
          >Punti GPS<input
            id="bm2-gps-points"
            v-model.number="form.gpsPoints"
            type="number"
            min="2"
        /></label>
      </div>
      <label class="bm2-check">
        <input id="bm2-simulation" v-model="isSimulation" type="checkbox" />
        Modalità simulazione ("what if")
      </label>
    ...
    <div v-if="answer" class="bm2-results">
      <article v-for="[name, r] in resultList()" :key="name" class="bm2-card">
        ...
        <dl>
          <dt>Risultato</dt>
          <dd>{{ r.value.toFixed(2) }} {{ r.unit }}</dd>
          <dt>Formula</dt>
          <dd>{{ r.formula }}</dd>
          <dt>Dati usati</dt>
      ...
      <aside v-if="answer.insights.length" class="bm2-insights">
        ...
        <ul>
          <li
            v-for="(ins, i) in answer.insights"
            :key="i"
    ...
    <section class="bm2-realride">
      ...
      <p class="bm2-hint">
        Applica il motore fisico BM2 a una tua uscita salvata per uno scenario
        "what if" o per validare la potenza stimata contro il power-meter.
      </p>
      <div class="bm2-row">
        <label
          >{{ t("bm2.ridePickerLabel") }}
          <div class="ride-picker">
            ...
            >
              {{
                rideForm.rideId
                  ? rides.find((r) => r.id === rideForm.rideId)?.date ||
                    `Ride #${rideForm.rideId}`
                  : "Seleziona una ride..."
              }}
            </button>
            <div v-if="ridePickerOpen" class="picker-dropdown">
              ...
              >
                ...
                <span class="picker-title">{{
                  ride.title || `Ride #${ride.id}`
                }}</span>
                <span class="picker-dist"
                  >{{ ride.distance_km?.toFixed(1) }} km</span
                >
              </div>
      ...
      <fieldset class="bm2-fieldset">
        ...
        <div class="bm2-row">
          <label
            >Δ peso atleta (kg)<input
              id="bm2-athlete-weight-delta"
              v-model.number="rideForm.athleteWeightDelta"
              type="number"
              step="0.5"
          /></label>
          <label
            >Δ peso bici (kg)<input
              id="bm2-bike-weight-delta"
              v-model.number="rideForm.bikeWeightDelta"
              type="number"
              step="0.5"
          /></label>
          <label
            >Δ pendenza (%)<input
              id="bm2-slope-delta"
              v-model.number="rideForm.slopeDelta"
              type="number"
              step="0.5"
          /></label>
          <label
            >CdA override<input
              id="bm2-cda-override"
              v-model.number="rideForm.cdaOverride"
              type="number"
              step="0.01"
              placeholder="es. 0.30"
          /></label>
        ...
        <button
          type="button"
          :disabled="loading || !rideForm.rideId"
          @click="onSimulateRide"
        >
      ...
      <fieldset class="bm2-fieldset">
        ...
        <div class="bm2-row">
          <label
            >Peso bici (kg)<input
              id="bm2-real-bike-weight"
              v-model.number="rideForm.bikeWeight"
              type="number"
              step="0.1"
          /></label>
          <label
      ...
      <div v-if="validation" class="bm2-results">
        ...
        <dl class="bm2-valgrid">
          <dt>Punti</dt>
          <dd>{{ validation.validation.n_points }}</dd>
          <dt>MAE</dt>
          <dd>{{ validation.validation.mae_w.toFixed(1) }} W</dd>
          <dt>RMSE</dt>
          <dd>{{ validation.validation.rmse_w.toFixed(1) }} W</dd>
          <dt>Bias</dt>
          <dd>{{ validation.validation.bias_w.toFixed(1) }} W</dd>
          <dt>Potenza media misurata</dt>
          <dd>{{ validation.validation.mean_measured_w.toFixed(1) }} W</dd>
          <dt>Potenza media stimata</dt>
          <dd>{{ validation.validation.mean_estimated_w.toFixed(1) }} W</dd>
          <dt>R²</dt>
          <dd>{{ validation.validation.r2.toFixed(3) }}</dd>
        </dl>
      </div>
    </section>
  </section>
