...
const trendData = ref({ ready: false, trend: "—", r2: 0, mean: 0, values: [], dates: [], rolling_avg: [] })
const monthlyData = ref({ ready: false, months: [], total_distance_km: [], avg_speed_kmh: [], total_duration_hours: [], ride_count: [] })
const comparisonData = ref({ ready: false, distance_change_pct: 0, speed_change_pct: 0, recent_rides: 0, previous_rides: 0 })
let trendChart = null
...
async function loadTrends() {
  try {
    const data = await apiGet(`/api/v1/analytics/trends?metric=${selectedMetric.value}&window=${windowSize.value}`)
  ...
  monthlyChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        y: { position: "left", ticks: { color: "#4ecca3" }, grid: { color: "rgba(255,255,255,0.05)", drawBorder: false } },
        y1: { position: "right", ticks: { color: "#FF6B00" }, grid: { display: false } },
      },
    },
  })
}
...
.trend-up { color: #4ecca3; font-weight: bold; }
