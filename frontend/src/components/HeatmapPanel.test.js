...
const mockHeatmap = {
  ...
  points: [
    ...
    { lat: 45.47, lon: 9.20, intensity: 0.6 },
  ],
}
...
describe('HeatmapPanel', () => {
  ...
  it('loads athlete ID on mount', async () => {
    apiGet.mockResolvedValueOnce(mockAthletes).mockResolvedValueOnce(mockHeatmap)
  ...
  it('shows empty state when no GPS data', async () => {
    apiGet.mockResolvedValueOnce(mockAthletes).mockResolvedValueOnce({ total_points: 0, points: [] })
    const wrapper = mount(HeatmapPanel)
    await flush()
  ...
  it('allows manual athlete ID input and triggers heatmap load', async () => {
    ...
    const heatmapCalls = apiGet.mock.calls.filter((c) => c[0] === '/api/v1/heatmap')
