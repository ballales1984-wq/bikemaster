...
//   3. ""  → stesso origine (default: in dev Vite fa proxy di /api a localhost:8000)
//
...
function normalizeBase(base: string): string {
  ...
  if (!trimmed) return "";
  // Rimuove lo slash finale per evitare doppi slash nell'URL.
...
// "local" per same-origin (dev).
export function getBackendMode(): "pc" | "render" | "local" {
