...
describe('validateAthleteForm', () => {
  ...
  it('rejects name too short', () => {
    const form = { name: 'A', age: 30, weight_kg: 70.0, experience_level: 'Beginner' }
    const errors = validateAthleteForm(form)
  ...
  it('accepts valid email', () => {
    const form = { name: 'Test', age: 30, weight_kg: 70.0, email: 'test@example.com' }
    const errors = validateAthleteForm(form)
    expect(errors.email).toBeUndefined()
  })

  it('rejects weight out of range', () => {
    const form = { name: 'Test', age: 30, weight_kg: 15.0, experience_level: 'Beginner' }
    const errors = validateAthleteForm(form)
    expect(errors.weight_kg).toBeDefined()
  })

  it('rejects age out of range', () => {
    const form = { name: 'Test', age: 9, weight_kg: 70.0, experience_level: 'Beginner' }
    const errors = validateAthleteForm(form)
    expect(errors.age).toBeDefined()
  })

  it('rejects invalid experience level', () => {
    const form = { name: 'Test', age: 30, weight_kg: 70.0, experience_level: 'Invalid' }
    const errors = validateAthleteForm(form)
    expect(errors.experience_level).toBeDefined()
  })

  it('rejects ftp out of range', () => {
    const form = { name: 'Test', age: 30, weight_kg: 70.0, ftp_watts: 10, experience_level: 'Beginner' }
    const errors = validateAthleteForm(form)
