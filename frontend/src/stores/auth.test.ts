...
describe('auth store', () => {
  ...
  it('parseJWTPayload decodes exp and claims', async () => {
    ...
    const payload = Buffer.from(JSON.stringify({ sub: 'user1', tenant_id: 7, is_admin: true, exp: 9999999999 })).toString('base64url')
  ...
  it('login stores token and user from response', async () => {
    ...
    const store = useAuthStore()
    const header = Buffer.from(JSON.stringify({ alg: 'HS256', typ: 'JWT' })).toString('base64url')
    const payload = Buffer.from(JSON.stringify({ sub: 'alice', tenant_id: 42, is_admin: false })).toString('base64url')
  ...
  it('login throws on 401', async () => {
    ...
    await expect(store.login('alice', 'bad')).rejects.toThrow('Invalid credentials')
    formSpy.mockRestore()
  })
